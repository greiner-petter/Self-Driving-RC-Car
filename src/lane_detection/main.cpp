            ...
            switch(ipc_packet.get_message_id())
            {
                case ocMessageId::Lines_Available:
                {
                   ...
                   /* ipc_packet.set_sender(ocMemberId::Lane_Detection);
                    ...
                    ipc_packet.clear_and_edit()
                        .write<int16_t>(36)
                        .write<int8_t>(car_properties.front_steering_angle_to_byte(getAverageOfOldAngles())/2)
                        .write<int8_t>(-car_properties.rear_steering_angle_to_byte(getAverageOfOldAngles())/2)
                        .write<uint8_t>(0x8)
