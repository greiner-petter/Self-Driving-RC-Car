        ...
        while (0 < (socket_status = socket->read_packet(ipc_packet, false)))
        {
            switch(ipc_packet.get_message_id())
            {
                case ocMessageId::Lines_Available:
                {
                    ...
                    float front_angle = -std::clamp<float>(std::asin(110 / pixelRadiusTocmRadius), -65, 65);
                    ...
                    logger->log("Radius: %d, rad in cm %f, sinErgebnis %f, ANGLE: %f, BANGLE: %f", radius, pixelRadiusTocmRadius, std::asin(110 / pixelRadiusTocmRadius) * (180/3.14),front_angle, back_angle);
