...
class simStopLineDetection : public simDetection
{
private:
...
public:
    void handle_packet(const ocPacket& packet) override
    {
        ...
        switch (packet.get_message_id())
        {
            ...
            case ocMessageId::Object_Found:
            {
                ...
                if (ocObjectType::Road_Stop_Line  == obj.object_type)
                {
                    _object = obj;
                    _object_found = 0 < obj.length;
                }
                if (ocObjectType::Road_Yield_Line == obj.object_type)
                {
                    _object = obj;
                    _object_found = 0 < obj.length;
                }
        ...
        const ocCarState&        car) override
    {
        ...
        {
            if (ocObjectType::Road_Stop_Line == obj.type ||
                ocObjectType::Road_Yield_Line == obj.type)
            {
                    ...
                    0.5f < std::abs(dot(car.pose.x_axis(), obj.pose.x_axis()))) // facing towards the car)
                {
                    ...
                    _object = {
                        ...
                        .distance_ahead = from_car.x - 2.0f,
                        .length         = 4.0f
                    };
                }
            }
        ...
        const ocCarState&        car) override
    {
        ...
        if (_object_found)
        {
            ...
            Vec2 p2 = (car_to_screen * Vec4(start,  22.0f, 0.0f, 1.0f)).xy();
            Vec2 p3 = (car_to_screen * Vec4(end,   -60.0f, 0.0f, 1.0f)).xy();
